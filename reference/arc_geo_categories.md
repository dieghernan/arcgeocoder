...
- Either to provide a pair of coordinates (`x,y` parameters) that would
...
- ...:
  ...
  :   The spatial reference of the `x,y` coordinates returned by a
      geocode request. By default is `NULL` (i.e. the parameter won't be
...
This function is vectorized over `category`, that means that it would
perform one independent call to
[`arc_geo()`](https://dieghernan.github.io/arcgeocoder/reference/arc_geo.md)
