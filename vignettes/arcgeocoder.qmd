...
geocoding with the ArcGIS REST API service. It accesses the ArcGIS REST API
with fewer dependencies, such as **curl**. In some situations, **curl** may not
be available or accessible, so **arcgeocoder** uses base functions to overcome
this limitation.
...
`full_results = TRUE` can be used to return all data from the geocoder service.
