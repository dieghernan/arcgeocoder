...
The API can return different results for the same `x,y` values depending on the
value of `featuretypes`. When `featuretypes = NULL`, the feature type returned
depends on the hierarchy shown in @tbl-hier.
