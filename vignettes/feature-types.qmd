...
Reverse geocoding converts coordinates into an address. The ArcGIS REST API
`reverseGeocode` operation selects the most relevant feature near the input
location by using a prioritized hierarchy of feature types.
...
the vector to the comma-separated value required by the API.
...
This example uses the default `featuretypes = NULL`. The input location is
within the search tolerance of both `POI` and `PointAddress` features, but the
API returns the higher-priority `POI` centroid (see @tbl-hier). The `Addr_type`
field identifies the returned feature type.
