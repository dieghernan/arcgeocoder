...
Unless otherwise noted, each feature type is returned only when the distance
...
You can narrow the query output to a specific feature type or a list of feature
types. The possible values supported for this argument are:
...
You can include several feature types. If more than one value is specified for
the argument, separate the values with commas and no spaces after each comma.
...
The input location is within the search tolerance of both `POI` and
...
@tbl-hier is still applied to the requested `featuretypes`.
...
The following example presents a case where only certain `featuretypes` are
near the requested location. In this case, when reverse geocoding the North
Pole, the API would return a `Locality`, but no `StreetAddress` is found.
...
on the `featuretypes` value. When `featuretypes = NULL`, the feature type
returned depends on the hierarchy shown in @tbl-hier.

Depending on the location, the `featuretypes` filter may not return results,
so using `featuretypes = NULL` is safer for general purposes.
